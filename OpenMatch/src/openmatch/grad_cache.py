...
class GradCache:
    ...
    def cache_step(
        ...
        # model_inputs = tuple({query: {input_ids:, attn_mask: }}, {passage: {input_ids:, attn_mask: }},  {cn_x: {input_ids:, attn_mask: }})

        model_inputs = [self.split_inputs(x, chunk_size) for x, chunk_size in zip(model_inputs, self.chunk_sizes)] # length=7
