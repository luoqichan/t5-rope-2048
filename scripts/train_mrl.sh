...
train_data_folder=$DATA_PATH/data/training_data/t5-rope-mrl
train_data=$train_data_folder/train.jsonl
valid_data=$train_data_folder/val.jsonl

trained_model_name=t5-rope-bkt-warmup-mrl
output_path=$DATA_PATH/models/$trained_model_name

WANDB_PROJECT=$trained_model_name

accelerate launch --num_processes $n_gpus --multi_gpu --main_process_port 29778 OpenMatch/src/openmatch/driver/train_dr.py  \
...
echo Execution time for build_index.py was `expr $end - $start` seconds.
...
echo Execution time for retrieve.py was `expr $end - $start` seconds.
...
echo Execution time for retrieve.py for dev queries was `expr $end - $start` seconds.
